...
- Release profile version: `0.1.10` (Android Version Code `11`)
- Development profile version: `0.1.09` (Android Version Code `10`) - not raised with the release profile
- Version snapshot: 2026-09-19. `0.1.10` is the first release build with the gacha work; the HUD rework merged after it and is in no release build yet. Profile-specific Player Settings override the project-wide version.
...
- SlimeInstance is a domain object; SlimeInstanceSaveData owns persistence mapping. Save schemas are Currency 2, SlimeStatus 8, and Upgrade 1. Preserve deterministic legacy migration IDs. All six repositories reject a stored version higher than the app supports.
...
- A list initializer that is already filled also changes local JSON loads. Newtonsoft's default `ObjectCreationHandling.Auto` appends to an existing list instead of replacing it, so `SlimeStatusSaveData`'s 20-slot collection lists came back 40 long and normalization kept only the empty first half: every local load wiped collection registrations and statistics. `PlayerPrefsSlimeStatusRepository` deserializes with `ObjectCreationHandling.Replace`; any new save type with a pre-filled list needs the same. Whether Firestore's `ConvertTo` has the same behavior has not been checked on a device.
...
- `OfflineRewardManager` computes the offline reward from `CurrencyManager.LastSaveTime` and per-slime auto-production, gated by a minimum interval, a maximum accrual window, and an efficiency factor. It raises `PresentationBlockChanged`, and `GameManager` keeps gameplay inactive through it until the reward popup is dismissed.
...
- `TutorialProgress` stores completion per user ID in PlayerPrefs and treats existing progress as completion of the main tutorial. DisplayRoom remains incomplete until its final dialogue.
- Completion is also recorded in the account document, `SlimeStatus.CompletedTutorials` (v8), because the local flags vanish when app data is cleared or the device changes. DisplayRoom and Gacha register with `completeByDefault: false`, so without the cloud record a reinstall replays both. A tutorial counts as complete when the local flag, the cloud record, or its default rule says so - the cloud record wins even over a stored local `0`, which covers finishing on another device. `TutorialProgress.MarkCompleted` writes both, and `GameManager` calls `UploadCompletedToCloud` after registration so completions made before v8 move into the document once. The cloud id follows the local key's version suffix, so bumping a tutorial's version in `TutorialIds.All` still replays it everywhere.
...
- That marker needs a Firestore rule allowing the owner to create or update only `ServerSyncTime`, and only when it equals `request.time`, while still allowing the owner to delete the marker during reset. Without the rule, login succeeds and GameScene entry is blocked; without the `request.time` check, a client can write its own timestamp and the whole defence is bypassed.
...
- A visible collected ticket preserves its projected position and size while it is reparented from the world-space ticket canvas to the scene-authored `TicketCollectOverlay` at canvas order 3. It bursts on touch, flies above slimes and the HUD into the top ticket icon, then bursts and punches the target before the save/currency transaction completes. Hidden tickets auto-collected from the other stage settle without being shown on the current stage.
...
- `GachaResultDirector` opens a portal, charges it, and lets the player tap it to reveal the slime. `NormalGachaPool` derives rarity from the selected candidate's actual weight rank and passes it through `GachaService` to the director, so changing the pool weights also changes the portal colour without a duplicate grade-distance rule. The draw is over before the portal appears, so the tap changes nothing. `GachaButtonUI` refuses a second pull while `IsPlaying`, because the first one would spend a ticket whose presentation is then rejected.
...
- `MergeManager` validates same-grade merges and promotes the surviving slime. It moves the save state first and only then raises the highest grade and updates the visuals, because `SlimeManager.MergeSlimesBatch()` throws on an entry the save does not have. Ordering the irreversible grade update after that check is what keeps a failure from leaving the save and the screen disagreeing, so no rollback is needed.
- `AutoMergeManager` runs the collection-10 auto merge: one interval, lowest grade first, and `MergeManager.MergeBatch` so the whole trigger saves once. It merges up to 1/2/3/4 pairs at AutoMergeTimeSub Lv.0/20/40/50, with every selected pair gathering simultaneously before the single batch. It skips dragged, special, DisplayRoom, top-grade and not-yet-landed slimes, and it stands down while a tutorial, the DisplayRoom send mode or the gacha result is on screen. `MergeBatch` returns whether anything merged: a pair the save refuses must not end the tick, or one unsaved slime blocks every other grade forever, and the refused ids are remembered so the retry does not pick them again. The batch is all-or-nothing, so when it is refused the manager retries each pair on its own; otherwise the valid pairs chosen alongside a refused one would be remembered as refused too and never auto-merge again that session.
- When no pair exists the timer stays full instead of restarting, and a short retry looks again, so a slime that spawns and lands - or a gacha result that reaches the field - merges as soon as it arrives. The next interval starts only after the presentation ends, never at the moment the pair was chosen.
- The presentation moves both slimes to their midpoint before merging, and locks only those two through `SlimeController.SetPresentationLocked` - colliders off so taps and drags miss them, rigidbody off so physics does not fight the tween. Unlock both afterwards, including the one that was despawned, because Lean Pool hands the same object out again.
- The merge effect is instantiated flat (X 90) and takes the keeper's sorting layer, like `MergeCandidateVfxController` does. A plane left upright renders edge-on and looks like nothing spawned at all.
...
- `UpgradeUI` derives its closed position from the actual panel width and applies `Screen.safeArea` insets. The drawer's top edge is the lower of the safe-area top and the top bar's bottom edge, read from `_topBar` at layout time, so points stay visible while upgrading. Layout refresh is event-driven through rect-size, focus, and pause callbacks; do not restore a fixed movement distance or per-frame layout polling.
- `BottomHudButtonLayout` places the panel switch button, the auto-spawn/auto-merge toggles and the gacha button at runtime, so their authored positions are overwritten. It computes where the bottom-left corner belongs (bottom-right for the gacha button, which mirrors the switch button on the right edge) and converts to `anchoredPosition` by adding `size * pivot`, because `anchoredPosition` addresses the pivot. Without that conversion, changing a pivot in the Inspector slides the button by half its size and correcting the authored value only hides it until the resolution or safe area changes. `BottomPanelSwitcher` owns panel selection and animation only.
- `BooleanToggleButtonView` owns shared ON/OFF icon and label presentation; feature toggle components own state changes and persistence. `RadialProgressView` presents the auto-merge timer as a border ring with a completion pulse through `RingProgressGraphic`, while `GachaHudVisibility` owns gacha and auto-merge feature-button visibility. Its targets are scene references only: an empty list logs an error and disables the component, because filling it by type or name lookup once hid an unwired auto-merge list. `BottomHudButtonLayout` also normalizes the four button labels to the shared 160x40 / 32pt presentation.
...
- `ScholarGuideUI` owns the scholar-slime information flow. It moves a scene-authored copy of the scholar to center, requests both HUD roots through `HudVisibility`, and blocks direct world input without pausing natural spawning, auto merge, or auto production. Background systems must not close or replace the guide; it stays open until its own back/close flow ends. Its sorting order is 50: above point floaters and ordinary HUD, below tutorial presentation and gacha. The old spawn-probability popup remains in the scene as disabled fallback UI; do not reconnect the scholar button directly to it. The separate system-upgrade status button and popup were deleted, so the scholar guide is the only place that shows upgrade status.
- Natural spawn probabilities and system-upgrade status text come from `GameplayInfoTextBuilder`. The scholar guide and the retained spawn-probability popup must use that shared builder so displayed levels, effects, and unlock filtering cannot drift. Which system upgrades appear at all is decided by `SystemUpgradeVisibility`, shared by the carousel and the builder; the higher-grade spawn card stays hidden until its tutorial is active or complete, so the guide must not list it earlier.
...
The offline reward, tutorial, exit popup, audio, Phase 2/2-B DisplayRoom, Phase 3's normal-slime collection book, game-account deletion, the save-layer hardening described above, and the scene-transition curtain have since shipped on `main`. Phase 4's gacha ticket merged to `main` through PR #23: the drop, the pending-ticket save, collection, the pull, the auto-spawn toggle, the tutorial, and the full-screen result presentation. The HUD rework merged through PR #24: the two-row top bar that holds points, tickets, options, the scholar slime and the spawn gauge, the upgrade drawer and card cleanup, the floater canvas, and the tutorial's action-prompt box. Phase 7 is implemented on `feature/phase-7-ticket-progression`: 10 registered normal slimes unlock the auto merge with its own 50-level interval upgrade, 12 auto-collect field tickets, 15 grant linearly floored offline tickets up to 5 per 8 hours, and 20 show the one-time main ending and unlock persisted hidden-fever misses. That branch also reworked the gacha result into the tap-to-open portal and moved `SlimeStatus` to 7, then to 8 for the cloud tutorial record; none of it has run in a release build. The phases that remain are 6, 8, then 5; Phase 5 must call `SlimeManager.RecordSpecialGachaResult()` from its special-result decision point.
...
`0.1.09` shipped to closed testing ahead of Phase 4 rather than after it, to close the gap between the work on `main` and the last release build. `0.1.10` (Android Version Code 11, built 2026-09-17 from the Phase 4 merge) is the first release build to run the gacha work, the write throttle, the pause flush, the loading timeout, and the DisplayRoom badge; its build-info checks carry no device result in this file yet. Two save schemas moved with it - Currency to 2 and SlimeStatus to 6 - so loading an existing document is the check that matters most on that build. The HUD rework came after it and has Editor Play Mode evidence only: notch clearance, the tutorial's highlight and prompt positions, and asset sharpness still need a device. Sign-in on that track failed for accounts that had never played the game before; the cause was Play Games Services tester registration, not the build - see `Documentation/GOOGLE_PLAY_FIREBASE_INTEGRATION.md` 6.4-1 before suspecting signing, device state, or the client.
