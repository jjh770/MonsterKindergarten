...
- `BooleanToggleButtonView` owns shared ON/OFF icon and label presentation; feature toggle components own state changes and persistence. `RadialProgressView` owns the auto-merge fill rendering, while `GachaHudVisibility` owns gacha and auto-merge feature-button visibility. `BottomHudButtonLayout` also normalizes the four button labels to the shared 160x40 / 32pt presentation.
