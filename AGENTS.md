...
- `BooleanToggleButtonView` owns shared ON/OFF icon and label presentation; feature toggle components own state changes and persistence. `RadialProgressView` presents the auto-merge timer as a border ring with a completion pulse through `RingProgressGraphic`, while `GachaHudVisibility` owns gacha and auto-merge feature-button visibility. `BottomHudButtonLayout` also normalizes the four button labels to the shared 160x40 / 32pt presentation.
...
- Natural spawn probabilities and system-upgrade status text come from `GameplayInfoTextBuilder`. The scholar guide and the retained legacy popups must use that shared builder so displayed levels, effects, and unlock filtering cannot drift.
