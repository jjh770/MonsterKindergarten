...
- `GachaResultDirector` opens a portal, charges it, and lets the player tap it to reveal the slime. The portal's colour comes from how far the result sits below the highest grade (minus 1 is the jackpot colour), so it reads as rarity without telling the player anything the pull has not already decided. The draw is over before the portal appears, so the tap changes nothing. `GachaButtonUI` refuses a second pull while `IsPlaying`, because the first one would spend a ticket whose presentation is then rejected.
