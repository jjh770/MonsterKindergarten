...
- `LoginScene` attempts silent Google Play sign-in first and supports an explicit manual retry.
...
- `_minimumCoveredSeconds` on the curtain and `_minimumVisibleSeconds` on `LoginScene` exist for the same reason in opposite directions. Editor logins and local loads finish in a few frames, so without them the curtain flickers past and the login screen never appears at all. Device logins already exceed both, so neither delays anything there.
