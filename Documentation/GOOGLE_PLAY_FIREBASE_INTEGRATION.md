...
기본값으로 바꿔 돌려주었는데, 그러면 실패한 세션이 신규 계정으로 시작하고 첫
저장이 읽지 못한 원본을 덮어썼습니다.

- `HybridRepository`는 어느 한쪽이라도 읽지 못하면 진행하지 않습니다. 로컬이
  손상돼도 클라우드를 읽었다면 그것으로 복구하고, 로컬이 현재 앱보다 상위 버전이면
  클라우드가 정상이어도 막습니다. 클라우드 채택 시의 로컬 사본 갱신은 보조 작업이라
  실패해도 로드를 중단시키지 않습니다.
- 여섯 저장소 모두 상위 저장 버전을 거부합니다. 이전에는 슬라임 저장소에만 있었습니다.
- 저장이 만들 수 없는 값은 변질로 보고 세션을 차단합니다. 복원 불가·중복 슬라임 개체,
  범위 밖 `HighestGrade`, 음수·NaN·무한대 재화, 길이가 다른 재화 배열, 열거형 범위
  밖 업그레이드 항목이 해당합니다.
- 세 문서는 함께 만들어지고 함께 지워지므로, 일부만 있는 상태는 신규 계정이 아니라
  결손으로 보고 차단합니다.
- 차단된 세션은 로그인 화면의 `SaveRecoveryUI`에서 초기화로 복구합니다. 손상·결손일
  때만 열립니다. 네트워크 실패는 재시도가, 상위 저장 버전은 앱 업데이트가 답이라
  그 두 경우에 초기화를 열면 멀쩡한 진행도를 지우게 됩니다.

기기 검증(개발 APK, `0.1.08 / Version Code 9`)에서 다음을 확인했습니다.

- `Currency/{UID}` 문서의 `Currencies` 필드만 삭제 → 게임 진입 없이 안내와 로그인 복귀.
- `Upgrade/{UID}` 문서의 `Entries` 필드만 삭제 → 동일.
- 장식장 튜토리얼 진행 중 백그라운드 후 복귀 → 오프라인 보상 팝업이 튜토리얼과 겹치지
  않고, 튜토리얼 종료 후 표시.

문서를 통째로 지운 경우는 필드 결손과 다른 경로(결손 세이브)를 타므로 로그로 구분됩니다.
`Currency : 재화 배열을 해석할 수 없습니다`는 필드 결손, `저장 문서가 일부만 있습니다`는
문서 결손입니다.

### 0.1.08 프로필 상태

- 릴리스 프로필: `0.1.08 / Version Code 9`, AAB, Development Build 꺼짐.
- 개발 프로필: `0.1.08 / Version Code 9`, APK, Development Build 켜짐.
- `build-info.txt` 핸드오프 기록은 `0.1.06`까지만 있습니다. `0.1.07`과 `0.1.08`은
  산출물만 있고 기록이 없습니다.
- `Builds/Release/0.1.08/`의 AAB는 2026-08-29 산출물이라 이 절의 저장 로드 실패
  처리가 들어 있지 않습니다. 위 검증은 별도로 만든 개발 APK로 수행했으며, 해당
  내용을 포함한 릴리스 AAB와 스토어 업로드는 아직 없습니다.
