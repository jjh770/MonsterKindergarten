...
> **후속 (2026-09-05):** 이 문단의 성능 판정은 그대로 유효합니다. 다만 `Default`를
> 반환하도록 한 수정은 이후 되돌렸습니다. 예외를 기본값으로 바꾸면 손상된 저장이
> 신규 계정으로 둔갑하고 첫 저장이 원본을 덮어쓰기 때문입니다. 지금은 `Load()`가
> 실패를 별도 상태로 올려 보내고 세션을 차단합니다. 경위는
> `CODE_REVIEW_2026-08-20.md` §1.1의 후속 항목에 있습니다.
