...
- 우측 상단 옵션의 배경음·효과음 조절과 현재 계정 진행도 초기화
...
| --- | --- | --- | ---: |
| Play Console 배포 | `Android_Release` | `0.1.04` | `5` |
...
위 값은 2026-08-27 프로필 기준입니다. Release는 Development Build가 꺼진
AAB이고 개발 프로필은 Development Build가 켜진 APK입니다. 현재 프로젝트
전역 버전 대신 실제 빌드에 사용하는 프로필 값을 확인해야 합니다.

로컬 산출물과 빌드 기록은 `Builds/Release/0.1.04/`에 있습니다.
`build-info.txt`에 변경사항·설정·AAB 해시·출시 노트를 기록하며, `Builds/`는
Git에서 제외됩니다. 파일 생성과 스토어 업로드·검증 완료는 별개입니다.
...
현재 `feature/phase-2-display-room` 브랜치에서는 Phase 1·1.5의 자연 스폰
성장·개체 저장 기반 위에 Phase 2 장식장과 Phase 2-B 관찰 UX를 구현했습니다.
후속으로 옵션·진행도 초기화, 하단 메뉴 정리, 장식장 튜토리얼 중단 후 재개와
상위 스폰 안내 시 업그레이드 패널 자동 표시를 반영했습니다.

2026-08-27에 `0.1.04` AAB 산출물을 확인했습니다. 코드·씬·빌드 프로필에
미커밋 변경이 있으므로 현재 HEAD만으로 빌드 상태를 재현할 수는 없습니다.

Unity Editor에서 장식장 해금, 공간 전환, 연속 입고, 중복 경고, 꺼내기,
저장 후 소속 복원, 정보 UI, 관찰 모드와 이동 중 카메라 추적을 확인했습니다.
이는 기존 사용자 확인 기록입니다. 최근 튜토리얼 재개·패널 표시 수정과
이번 AAB의 Android 검증·스토어 업로드 결과는 아직 확인되지 않았습니다.
