...
- 일반 도감 10종을 등록하면 자동 합성이 열립니다. 버튼 뒤 원형 게이지로 다음
  발동까지의 주기를 확인하고, 전용 업그레이드로 20초에서 10초까지 0.2초씩
  단축합니다. 발동할 때마다 필드 전체에서 일반 슬라임 한 쌍만 합성하며, 두 마리가
  서로에게 모여 합쳐지는 연출이 나옵니다.
