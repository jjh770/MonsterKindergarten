...
- 일반 도감 10종을 등록하면 자동 합성이 열립니다. 버튼 테두리를 따라 차오르는
  게이지로 다음 발동까지의 주기를 확인하고, 전용 업그레이드로 20초에서 10초까지
  0.2초씩 단축합니다. 발동할 때마다 필드 전체에서 낮은 등급부터 일반 슬라임을
  합성하며, 한 번에 합성하는 쌍은 업그레이드 Lv.20·40·50에서 2·3·4쌍으로 늘어납니다.
  두 마리가 서로에게 모여 합쳐지는 연출이 나옵니다.
- 도감 12종부터 떨어진 가챠권을 자동으로 줍고, 15종부터는 접속하지 않은 시간에
  비례해 가챠권을 받습니다. 20종을 모두 모으면 졸업 연출이 한 번 나옵니다.
