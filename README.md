...
- 같은 등급 두 마리를 드래그해 합치면 다음 등급으로 성장하며, 최대 20등급까지
  있습니다.
- 모든 등급의 슬라임이 한 필드에서 함께 지냅니다. 10등급 둘을 합쳐 처음 11등급을
  만들면 하늘 배경이 열리고, 땅과 하늘 중에 배경을 고를 수 있습니다.
- 최고 등급 3부터 장식장이 열립니다. 메인 필드의 슬라임을 전시하거나 다시
...
- 최고 등급 7부터 필드에 가챠권이 떨어집니다. 주운 가챠권으로 최고 등급보다 1–4
  낮은 슬라임을 뽑을 수 있고, 자동 스폰을 끄고 켤 수 있습니다. 결과는 포털을
  터치해 여는 연출로 공개되며, 포털 색이 뽑힌 등급대를 알려 줍니다.
- 일반 도감 10종을 등록하면 자동 합성 버튼이 열립니다. 누를 때마다 필드 전체에서
  낮은 등급부터 일반 슬라임을 합성하고, 두 마리가 서로에게 모여 합쳐지는 연출이
  나옵니다. 한 번에 합성하는 쌍은 전용 업그레이드로 1쌍에서 10쌍까지 늘어납니다.
  버튼 테두리를 따라 차오르는 게이지로 다시 누를 수 있는 시점을 확인합니다.
- 도감 12종부터 떨어진 가챠권을 버튼 하나로 한 번에 회수하고, 15종부터는 접속하지
  않은 시간에 비례해 가챠권을 받습니다. 20종을 모두 모으면 졸업 연출이 한 번
  나옵니다.
...
슬라임 도감, Phase 4 가챠권, Phase 6 자동 합성, Phase 7 도감 마일스톤을
`main`에 병합했습니다. HUD 개편과 도감 12종 자동 티켓 수집, 15종 오프라인
가챠권, 20종 엔딩과 숨겨진 피버 저장 기반은 `0.1.11`에 포함되어 Play
Console 비공개 테스트 검토 중입니다. 현재는 Phase 8 UX·튜토리얼·밸런싱·QA를
완료했으며, 특별 슬라임과 특별 가챠는 후속 Phase 5 범위로 남깁니다.
